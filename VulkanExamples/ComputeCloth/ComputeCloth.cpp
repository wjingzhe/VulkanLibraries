...
public:
	...
	// REsource for the graphics part of the example
	struct GraphicPass
	{
		VkDescriptorSetLayout descriptorSetLayout;
		VkDescriptorSet desccriptorSet;
	...
	struct ComputePass
	{
		...
		VkDescriptorSetLayout descriptorSetLayout;
		std::array<VkDescriptorSet, 2> descritptorSets;
		VkPipelineLayout pipelineLayout;
		VkPipeline pipeline;
	...
	void prepareStorageBuffers()
	{
		...
		case 1:
		{
			...
			for (uint32_t i = 0; i < cloth.gridsize.y; i++)
			{
				for (uint32_t j = 0; j < cloth.gridsize.x; j++)
				{
					...
					// Pin some particles
					particleBuffer[i + j * cloth.gridsize.y].pinned = (i == 0) &&
		...
		vulkanDevice->CreateBuffer(VK_BUFFER_USAGE_VERTEX_BUFFER_BIT | VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT,
			VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, &compute.storageBuffers.input, storageBufferSize);

		vulkanDevice->CreateBuffer(VK_BUFFER_USAGE_VERTEX_BUFFER_BIT | VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT,
			VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, &compute.storageBuffers.output, storageBufferSize);
		...
		vulkanDevice->CreateBuffer(VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, &stagingBuffer, indexBufferSize, indices.data());
		vulkanDevice->CreateBuffer(VK_BUFFER_USAGE_INDEX_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, &graphics.indices, indexBufferSize);
	...
	void updateComputeUBO()
	{
		if (!paused)
		{
			...
			// complete.ubo.deltaT = frameTimer*0.0075f;
	...
	void prepareUniformBuffers()
	{
		...
		compute.ubo.restDistD = sqrt(dx*dx + dy * dy);
	...
	void setupLayoutAndDescriptors()
	{
		...
		// Set
		VkDescriptorSetAllocateInfo allocInfo = vks::initializers::GenDescriptorSetAllocateInfo(descriptorPool, &graphics.descriptorSetLayout, 1);
		VK_CHECK_RESULT(vkAllocateDescriptorSets(device, &allocInfo, &graphics.desccriptorSet));

		std::vector<VkWriteDescriptorSet> writeDescriptorSets =
		{
			vks::initializers::GenWriteDescriptorSet(graphics.desccriptorSet,VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER,0,&graphics.uniformBuffer.descriptorBufferInfo),
			vks::initializers::GenWriteDescriptorSet(graphics.desccriptorSet,VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER,1,&textureCloth.descriptorImageInfo),
		};
	...
	{
		VkPipelineInputAssemblyStateCreateInfo inputAssemblyStateCI = vks::initializers::GenPipelineInputAssemblyStateCreateInfo(VK_PRIMITIVE_TOPOLOGY_TRIANGLE_STRIP, 0, VK_TRUE);
		...
		VkPipelineDynamicStateCreateInfo dynamicStateCI = vks::initializers::GenPipelineDynamicStateCreateInfo(dynamicStateEnables, 0);

		// Rendering pipeline
		std::array<VkPipelineShaderStageCreateInfo, 2> shaderStages;
		shaderStages[0] = loadShader(getShaderPath() + "computecloth/cloth.vert.spv", VK_SHADER_STAGE_VERTEX_BIT);
		shaderStages[1] = loadShader(getShaderPath() + "computecloth/cloth.frag.spv", VK_SHADER_STAGE_FRAGMENT_BIT);

		VkGraphicsPipelineCreateInfo pipelineCreateInfo = vks::initializers::GenPipelineCreateInfo(graphics.pipelineLayout, renderPass);
		...
		std::vector<VkVertexInputAttributeDescription> inputAttributes =
		{
			...
			vks::initializers::GenVertexInputAttributeDescription(0,1,VK_FORMAT_R32G32_SFLOAT,offsetof(Particle,uv)),
			vks::initializers::GenVertexInputAttributeDescription(0,2,VK_FORMAT_R32G32B32A32_SFLOAT,offsetof(Particle,normal))
		};

		// Assign to vertex buffer
		VkPipelineVertexInputStateCreateInfo inputStateCI = vks::initializers::GenPipelineVertexInputStateCreateInfo();
		inputStateCI.vertexBindingDescriptionCount = static_cast<uint32_t>(inputBindings.size());
		inputStateCI.pVertexBindingDescriptions = inputBindings.data();
		inputStateCI.vertexAttributeDescriptionCount = static_cast<uint32_t>(inputAttributes.size());
		inputStateCI.pVertexAttributeDescriptions = inputAttributes.data();

		pipelineCreateInfo.pVertexInputState = &inputStateCI;
		...
		shaderStages[0] = loadShader(getShaderPath() + "computecloth/sphere.vert.spv", VK_SHADER_STAGE_VERTEX_BIT);
		shaderStages[1] = loadShader(getShaderPath() + "computecloth/sphere.frag.spv", VK_SHADER_STAGE_FRAGMENT_BIT);
	...
	void addGraphicsToComputeBarriers(VkCommandBuffer commandBuffer)
	{
		if (specializedComputeQueue)
		{
			...
			vkCmdPipelineBarrier(commandBuffer, VK_PIPELINE_STAGE_VERTEX_INPUT_BIT, VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT,
				VK_FLAGS_NONE, 0, nullptr, static_cast<uint32_t>(bufferBarriers.size()), bufferBarriers.data(), 0, nullptr);
		}
	}
	...
	// todo: check barriers (validation,separate complete queue)
	void buildComputeCommandBuffer()
	{
		...
		for (uint32_t i = 0; i < 2; i++)
		{
			...
			// Dispatch the complete job
			const uint32_t iterations = 64;
			for (int32_t j = 0; j < iterations; j++)
			{
				readSet = 1 - readSet;
				vkCmdBindDescriptorSets(compute.commandBuffers[i], VK_PIPELINE_BIND_POINT_COMPUTE, compute.pipelineLayout, 0, 1, &compute.descritptorSets[readSet], 0, 0);
	...
	void prepareCompute()
	{
		//Create a complete capable device queue
		...
		std::vector<VkDescriptorSetLayoutBinding> setLayoutBindings =
		{
			...
			vks::initializers::GenDescriptorSetLayoutBinding(VK_DESCRIPTOR_TYPE_STORAGE_BUFFER,VK_SHADER_STAGE_COMPUTE_BIT,1),
			vks::initializers::GenDescriptorSetLayoutBinding(VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER,VK_SHADER_STAGE_COMPUTE_BIT,2),
		};

		VkDescriptorSetLayoutCreateInfo descriptorLayoutCI = vks::initializers::GenDescriptorSetLayoutCreateInfo(setLayoutBindings);
		VK_CHECK_RESULT(vkCreateDescriptorSetLayout(device, &descriptorLayoutCI, nullptr, &compute.descriptorSetLayout));

		VkPipelineLayoutCreateInfo pipelineLayoutCI = vks::initializers::GenPipelineLayoutCreateInfo(&compute.descriptorSetLayout, 1);
		// Push constants used to pass some parameters
		VkPushConstantRange pushConstantRange = vks::initializers::GenPushConstantRange(VK_SHADER_STAGE_COMPUTE_BIT, sizeof(uint32_t), 0);
		pipelineLayoutCI.pushConstantRangeCount = 1;
		pipelineLayoutCI.pPushConstantRanges = &pushConstantRange;
		VK_CHECK_RESULT(vkCreatePipelineLayout(device, &pipelineLayoutCI, nullptr, &compute.pipelineLayout));

		VkDescriptorSetAllocateInfo allocateInfo = vks::initializers::GenDescriptorSetAllocateInfo(descriptorPool, &compute.descriptorSetLayout, 1);
		// Create two desriptor sets with input and output buffers switched
		VK_CHECK_RESULT(vkAllocateDescriptorSets(device, &allocateInfo, &compute.descritptorSets[0]));
		VK_CHECK_RESULT(vkAllocateDescriptorSets(device, &allocateInfo, &compute.descritptorSets[1]));

		std::vector<VkWriteDescriptorSet> computeWriteDescriptorSets =
		{
			vks::initializers::GenWriteDescriptorSet(compute.descritptorSets[0],VK_DESCRIPTOR_TYPE_STORAGE_BUFFER,0,&compute.storageBuffers.input.descriptorBufferInfo),
			vks::initializers::GenWriteDescriptorSet(compute.descritptorSets[0],VK_DESCRIPTOR_TYPE_STORAGE_BUFFER,1,&compute.storageBuffers.output.descriptorBufferInfo),
			vks::initializers::GenWriteDescriptorSet(compute.descritptorSets[0],VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER,2,&compute.uniformBuffer.descriptorBufferInfo),

			vks::initializers::GenWriteDescriptorSet(compute.descritptorSets[1],VK_DESCRIPTOR_TYPE_STORAGE_BUFFER,0,&compute.storageBuffers.output.descriptorBufferInfo),
			vks::initializers::GenWriteDescriptorSet(compute.descritptorSets[1],VK_DESCRIPTOR_TYPE_STORAGE_BUFFER,1,&compute.storageBuffers.input.descriptorBufferInfo),
			vks::initializers::GenWriteDescriptorSet(compute.descritptorSets[1],VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER,2,&compute.uniformBuffer.descriptorBufferInfo),
		};
		...
		computePipelineCreateInfo.stage = loadShader(getShaderPath() + "computecloth/cloth.comp.spv", VK_SHADER_STAGE_COMPUTE_BIT);
		VK_CHECK_RESULT(vkCreateComputePipelines(device, pipelineCache, 1, &computePipelineCreateInfo, nullptr, &compute.pipeline));

		// Separate command pool as queue family for complete may be different than graphics
		...
		// Create a command buffer for complete operations
		VkCommandBufferAllocateInfo cmdBufAllocateInfo = vks::initializers::GenCommandBufferAllocateInfo(compute.commandPool, VK_COMMAND_BUFFER_LEVEL_PRIMARY, 2);
		VK_CHECK_RESULT(vkAllocateCommandBuffers(device, &cmdBufAllocateInfo, &compute.commandBuffers[0]));

		// Semaphores for graphics / complete synchronization
	...
	void buildCommandBuffersForPreRenderPrmitives()
	{
		...
		for (int32_t i = 0; i < drawCmdBuffers.size(); i++)
		{
			...
			// Acquire storage buffers from complete queue
			...
			if (sceneSetup ==0)
			{
				...
				vkCmdBindDescriptorSets(drawCmdBuffers[i], VK_PIPELINE_BIND_POINT_GRAPHICS, graphics.pipelineLayout, 0, 1, &graphics.desccriptorSet, 0, nullptr);
			...
			// release the storage buffers to the complete queue
	...
	void prepareForRendering()
	{
		...
		// Make sure the code works properly both with differrent queues families for graphics and complete and the same queue family
...
#endif // DEBUG_FORCE_SHARED_GRAPHICS_COMPUTE_QUEUE
		// Check whether the complete queue family is distinct from the graphics queue family
		specializedComputeQueue = vulkanDevice->queueFamilyIndices.graphicIndex != vulkanDevice->queueFamilyIndices.computeIndex;
		loadAssets();
		prepareStorageBuffers();
		prepareUniformBuffers();
		setupDescriptorPool();
		setupLayoutAndDescriptors();
		preparePipelines();
		prepareCompute();
	...
	void draw()
	{
		...
		VkSemaphore waitSemaphores[2] = { semaphores.presentComplete,compute.semaphores.complete };

		VkSemaphore signalSemaphores[2] = { semaphores.renderComplete,compute.semaphores.ready };
