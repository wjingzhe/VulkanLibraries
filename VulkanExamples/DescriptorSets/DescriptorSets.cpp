...
public:
	...
	VkPipeline pipeline;
	VkPipelineLayout pipelineLayout;//描述不同时期的pass设置相关信息

	VkDescriptorSetLayout descriptorSetLayout;//描述各个shader中的const buffer Set组成结构信息

	VulkanExample() :VulkanExampleBase(ENABLE_VALIDATION)
	{
		windowTitle = "Using Descritor Sets";
	...
	void buildCommandBuffersForPreRenderPrmitives() override
	{
		...
		for (int32_t i = 0; i < drawCmdBuffers.size(); i++)
		{
			...
			model.bindBuffers(drawCmdBuffers[i]);//设置顶点buffer和indeBuffer
			...
			*/
			for (auto cube:cubes)
			{
				// Bind the cube's descriptor set. This tells the command buffer to use the uniform buffer and images set for this cube
	...
	void setupDescriptSets()
	{
		...
		*/
		...
		// Accssible from the vertex shader only (flags can be combined to make it accessible to multiple shader stages)
		...
		*/
		...
		/*
			...
			An application can have multiple pools (e.g. for multiple threads) with any number of descriptor types as long as device limits are not supported

			it's good pratices to allocate pools with actually required descriptor types and counts
		...
		*/
		...
		descriptorPoolSize[0].descriptorCount = 1 + static_cast<uint32_t>(cubes.size());
		...
		/*
		 Descriptor sets
		 Using the shared descriptor set layout and descriptor pool we will now allocate the descriptor sets
		...
		*/
		for (auto &cube:cubes )
		{
			...
			// Allocate an empty descriptor set without actual descriptors matching shader binding set in the layout
			...
			*/
			...
			// This is possible because each VkWriteDescriptorSet also contains the desctination set to be updated
	...
	void preparePipelines()
	{
		...
		*/
		...
		pipelineCI.pVertexInputState = vkglTF::Vertex::getPipelineVertexInputState(
			{vkglTF::VertexComponent::Position,vkglTF::VertexComponent::Normal,vkglTF::VertexComponent::UV,vkglTF::VertexComponent::Color});

		shaderStages[0] = loadShader(getShaderPath() + "descriptorsets/cube.vert.spv", VK_SHADER_STAGE_VERTEX_BIT);
		shaderStages[1] = loadShader(getShaderPath() + "descriptorsets/cube.frag.spv", VK_SHADER_STAGE_FRAGMENT_BIT);
	...
	void prepareUniformBuffers()
	{
		...
		for (auto& cube:cubes)
		{
			VK_CHECK_RESULT(vulkanDevice->CreateBuffer(VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, 
				VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, &cube.uniformBuffer, sizeof(Cube::Matrices)));
	...
	void updateUniformBuffers()
	{
		...
		cubes[1].matrices.model = glm::translate(glm::mat4(1.0f), glm::vec3(1.0f, 0.5f, 0.0f));
